...
bool Utils::check_delaunay_crit(Utils::edge * a)
{
	...
	if ((t2->a2->s2.x - c.c.x) * (t2->a2->s2.x - c.c.x) + (t2->a2->s2.y - c.c.y) * (t2->a2->s2.y - c.c.y) < c.r * c.r) {
		return false;
	}

	Utils::cercle c = get_circumscribed_circle(t2);
	if ((t1->a2->s2.x - c.c.x) * (t1->a2->s2.x - c.c.x) + (t1->a2->s2.y - c.c.y) * (t1->a2->s2.y - c.c.y) < c.r * c.r) {
		return false;
	}
